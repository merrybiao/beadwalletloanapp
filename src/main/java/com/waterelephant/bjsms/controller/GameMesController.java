...
@Controller
public class GameMesController {
	
	private Logger logger = Logger.getLogger(GameMesController.class);
	
	@ResponseBody
	@RequestMapping(value = "/sedGameMessage.do", method = RequestMethod.POST)
	public DefaultResponse sedGameMessage(@RequestBody RequestSmsData data) {
		...
		try {
			...
			if(RedisUtils.exists("system:gameSing")){
				Set<String> keys = RedisUtils.hkeys("system:gameSing");
				Iterator<String> iterator = keys.iterator();
				boolean flag = false;
				while(iterator.hasNext()){
					//所有的key
					String keyval = iterator.next();
					String sign =  RedisUtils.hget("system:gameSing", keyval);
				...
				if(flag){
					RedisUtils.rpush("system:game_sendMessage", JSON.toJSONString(data.getContent()));
				} else {
