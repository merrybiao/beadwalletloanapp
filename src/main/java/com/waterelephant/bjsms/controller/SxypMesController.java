...
@RequestMapping("sxyp/message")
@Controller
public class SxypMesController {

	private Logger logger = LoggerFactory.getLogger(SxypMesController.class);
	...
	public DefaultResponse sedSxypMessage(@RequestBody RequestSmsData data) {
		...
		try {
			...
			logger.info("【水象优品】加密的key为" + key);
			if (!data.getSecretkey().equals(key)) {
				response.setRequestCode("400");
				response.setRequestMsg("非法访问");
				return response;
			}
			if(RedisUtils.exists("system:sxypSing")){
				Set<String> keys = RedisUtils.hkeys("system:sxypSing");
				Iterator<String> iterator = keys.iterator();
				boolean flag = false;
				while(iterator.hasNext()){
					//所有的key
					String keyval = iterator.next();
					String sign =  RedisUtils.hget("system:sxypSing", keyval);
				...
				if(flag){
					logger.info("【水象优品】存入redis中的值为"+JSON.toJSONString(data.getContent()));
					RedisUtils.rpush("system:sxyp_sendMessage", JSON.toJSONString(data.getContent()));
				} else {
