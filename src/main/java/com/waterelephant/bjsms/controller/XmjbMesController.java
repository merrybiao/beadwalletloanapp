...
@RequestMapping("xmjb/message")
@Controller
public class XmjbMesController {

private Logger logger = Logger.getLogger(XmjbMesController.class);
	
	@ResponseBody
	@RequestMapping(value = "/sedXmjbMessage.do", method = RequestMethod.POST)
	public DefaultResponse sedXmjbMessage(@RequestBody RequestSmsData data) {
		...
		try {
			...
			logger.info("【厦门借宝】加密的key为" + key);
			if (!data.getSecretkey().equals(key)) {
				response.setRequestCode("400");
				response.setRequestMsg("非法访问");
				return response;
			}
			if(RedisUtils.exists("system:xmjbSing")){
				Set<String> keys = RedisUtils.hkeys("system:xmjbSing");
				Iterator<String> iterator = keys.iterator();
				boolean flag = false;
				while(iterator.hasNext()){
					//所有的key
					String keyval = iterator.next();
					String sign =  RedisUtils.hget("system:xmjbSing", keyval);
				...
				if(flag){
					RedisUtils.rpush("system:xmjb_sendMessage", JSON.toJSONString(data.getContent()));
				} else {
