 ...
 */
@Controller
@RequestMapping("/baofooQfq")
public class BaofooQfqController {
	private Logger logger = Logger.getLogger(BaofooQfqController.class);
	...
	public String loanAdvicePush(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			String pub_key = SystemConstant.CAPITAL_BAOFOO_QFQ_PUBKEYPATH;
			...
			System.out.println("解密后结果：" + reslut);
			...
			if (orderNo.startsWith("BJQFQ")) {
				systemQfqCapitalCentersService.updateCapitalBaofooNotity(orderNo, state, loanDate,
						StringUtil.toString(paramsMap.get("result")));
				return "OK";
			}
			if (orderNo.startsWith("77_")) {
				return "OK";
			}
