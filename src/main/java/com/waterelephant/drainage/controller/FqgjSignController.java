...
@Controller
@RequestMapping("/app/fqgj")
public class FqgjSignController extends BaseController {

	private static LogUtil logger = new LogUtil(FqgjSignController.class);
	...
	private static String ORDER_NO_SESSION = "fqgjOrderNo";
	...
	public void signRongCard(HttpServletRequest request, HttpServletResponse response) {
		String methodName = "FqgjSignController.signRongCard";
		...
		try {
			...
			String urlReturn = SystemConstant.NOTIRY_URL + "/app/fqgj/signCardCallBack.do";
			...
			RedisUtils.hset("fqgj:session", session.getId(), orderNo);
	 ...
	 * @return
	 */
	@RequestMapping(value = "/signCardCallBack.do")
	public String signRongCardCallBack(HttpServletRequest request, HttpServletResponse response, ModelMap map) {
		...
		try {
			...
			String thirdUrl = RedisUtils.hget("fqgj:order"+userId, "fqgj");
			...
			if (StringUtils.isBlank(orderNo)) {
				logger.info("start get orderNo from redis[fqgj:session]");
				orderNo = RedisUtils.hget("fqgj:session", session.getId());
				logger.info("end get orderNo from redis[fqgj:session] value is "+orderNo);
			}

			RedisUtils.hdel("fqgj:session", session.getId());
