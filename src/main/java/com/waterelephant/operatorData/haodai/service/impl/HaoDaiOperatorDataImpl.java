package com.waterelephant.operatorData.haodai.service.impl;
...
import com.waterelephant.operatorData.haodai.mapper.HaoDaiOperatorDataMapper;
import com.waterelephant.operatorData.service.impl.OperatorsDataAbstractService;
import com.waterelephant.utils.DateUtil;
@Service
public class HaoDaiOperatorDataImpl extends OperatorsDataAbstractService{
	
	private Logger logger = LoggerFactory.getLogger(HaoDaiOperatorDataImpl.class);
	
	@Autowired
	private HaoDaiOperatorDataMapper mapper;
	
	@Override
	public OperatorUserDataDto getUserData(Long borrowerId, Long orderId) {
		logger.info("----【好贷】----依据orderId:{}查询用户基本信息",orderId);
	...
	public List<OperatorMsgDataDto> getMsgDataList(Long borrowerId, Long orderId) {
		logger.info("----【好贷】根据orderId:{}获取短信记录信息----",orderId);
		int months = 6;
		...
		try {
			while(months > 0){
				...
				if(null != dto&&dto.getTotalSize() >0 ){
					...
					logger.info("----【好贷】根据orderId:{},startTime:{},lastTime:{},查询bw_haodai_sms表获取短信记录信息,结果为：{}",orderId,startTime,endTime,msgrecord == null ?"空":msgrecord.size());
			...
			logger.error("----【好贷】根据orderId:{}获取短信记录信息异常：{}----",orderId,e.getMessage());
			e.printStackTrace();
		}
		return list;
	}
	...
	public List<OperatorTelDataDto> getTelDataList(Long borrowerId, Long orderId) {
		logger.info("----【好贷】----根据orderId:{}获取通话记录信息----",orderId);
		int months = 6;
		Calendar calendar = Calendar.getInstance();
		...
		try {
			while(months>0){
				...
				if(null !=dto&&dto.getTotalSize()>0){
					...
					logger.info("----【好贷】----根据orderId:{},startTime:{},endTime:{},查询bw_haodai_call表获取通话记录信息,结果为：{}",orderId,startTime,endTime,items == null ?"空":items.size());
			...
			logger.error("----【好贷】----根据orderId:{}获取通话记录信息异常：{}----",orderId,e.getMessage());
			e.printStackTrace();
		}
		return list;
	}
	...
	public List<OperatorBillDataDto> getBillDataList(Long borrowerId,
			Long orderId) {
		logger.info("----根据【好贷】orderId:{}获取账单记录信息----",orderId);
		...
		try {
			...
			logger.info("----【好贷】根据orderId:{},startTime:{},endTime:{},查询bw_haodai_bill表获取账单记录信息,结果为：{}",orderId,startTime,endTime,listdata == null ?"空":listdata.size());
			...
			logger.error("----【好贷】根据orderId:{}获取账单信息异常：{}----",orderId,e.getMessage());
			e.printStackTrace();
		}
		return new ArrayList<OperatorBillDataDto>();		}
	...
	public List<OperatorNetDataDto> getNetDataList(Long borrowerId, Long orderId) {
		logger.info("----【好贷】根据orderId:{}获取流量记录信息----",orderId);
		int months = 6;
		Calendar calendar = Calendar.getInstance();
		...
		try {
			while(months>0){
				...
				logger.info("----【好贷】根据orderId:{},startTime:{},lastTime:{},查询bw_haodai_net表获取流量记录信息,结果为：{}",orderId,startTime,endTime,items == null ?"空":items.size());
				if(null != items&&items.size()>0){
					dto.setItems(items);
					list.add(dto);
				}
			...
			logger.error("----【好贷】根据orderId:{}获取流量记录信息异常：{}----",orderId,e.getMessage());
			e.printStackTrace();
		}
		return list;
	}
