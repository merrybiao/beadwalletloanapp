package com.waterelephant.operatorData.kabao.service.impl;
...
import com.waterelephant.operatorData.kabao.mapper.KaBaoOperatorDataMapper;
import com.waterelephant.operatorData.service.impl.OperatorsDataAbstractService;
import com.waterelephant.utils.DateUtil;
@Service
public class KaBaoOperatorDataImpl extends OperatorsDataAbstractService{
	
	private Logger logger = LoggerFactory.getLogger(KaBaoOperatorDataImpl.class);
	
	@Autowired
	private KaBaoOperatorDataMapper mapper;

	@Override
	public OperatorUserDataDto getUserData(Long borrowerId, Long orderId) {
		logger.info("----【51卡宝】----依据orderId:{}查询用户基本信息",orderId);
	...
	public List<OperatorRechargeDataDto> getRechargeDataList(Long borrowerId,
		...
		logger.info("----【51卡宝】根据borrowerId:{},orderId:{}获取充值记录信息----",borrowerId,orderId);
		...
		try {
			...
			logger.error("----【51卡宝】根据orderId:{}获取短信记录信息异常：{}----",orderId,e.getMessage());
		}
		logger.info("----【51卡宝】根据orderId:{},startTime:{},lastTime:{},查询bw_mx_recharge表获取充值记录信息,结果为：{}",orderId,startTime,endTime,list == null ?"空":list.size());
	...
	public List<OperatorMsgDataDto> getMsgDataList(Long borrowerId, Long orderId) {
		logger.info("----【51卡宝】根据orderId:{}获取短信记录信息----",orderId);
		int months = 6;
		Calendar calendar = Calendar.getInstance(); 
		...
		try {
			while(months > 0){
				...
				if(null != dto&&dto.getTotalSize() >0 ){
					...
					logger.info("----【51卡宝】根据orderId:{},startTime:{},lastTime:{},查询bw_mx_sms表获取短信记录信息,结果为：{}",orderId,startTime,endTime,msgrecord == null ?"空":msgrecord.size());
	...
	public List<OperatorTelDataDto> getTelDataList(Long borrowerId, Long orderId) {
		logger.info("----【51卡宝】----根据orderId:{}获取通话记录信息----",orderId);
		int months = 6;
		Calendar calendar = Calendar.getInstance();
		...
		try {
			while(months>0){
				...
				if(null !=dto&&dto.getTotalSize()>0){
					...
					logger.info("----【51卡宝】----根据orderId:{},startTime:{},endTime:{},查询bw_third_operate_voice表获取通话记录信息,结果为：{}",orderId,startTime,endTime,items == null ?"空":items.size());
			...
			logger.error("----【51卡宝】----根据orderId:{}获取通话记录信息异常：{}----",orderId,e.getMessage());
			e.printStackTrace();
		}
		return list;
	}
	...
	public List<OperatorBillDataDto> getBillDataList(Long borrowerId,
			Long orderId) {
		logger.info("----【51卡宝】----根据orderId:{}获取账单记录信息----",orderId);
		Calendar calendar = Calendar.getInstance();
		...
		try {
			...
			logger.info("----【51卡宝】----根据orderId:{},startTime:{},endTime:{},查询bw_mx_bills表获取账单记录信息,结果为：{}",orderId,startTime,endTime,list == null ?"空":list.size());
			if(null != list&&list.size()>0){
				return list;
			} else {
			...
			logger.error("----【51卡宝】----根据orderId:{}获取账单记录信息异常：{}----",orderId,e.getMessage());
			e.printStackTrace();
		}
		return null;
	}
	...
	public List<OperatorNetDataDto> getNetDataList(Long borrowerId, Long orderId) {
		logger.info("----【51卡宝】根据orderId:{}获取流量记录信息----",orderId);
		int months = 6;
		Calendar calendar = Calendar.getInstance();
		...
		try {
			while(months>0){
				...
				logger.info("----【51卡宝】根据orderId:{},startTime:{},lastTime:{},查询bw_mx_net表获取流量记录信息,结果为：{}",orderId,startTime,endTime,items == null ?"空":items.size());
				if(null != items&&items.size()>0){
					dto.setItems(items);
					list.add(dto);
				}
			...
			logger.error("----【51卡宝】根据orderId:{}获取流量记录信息异常：{}----",orderId,e.getMessage());
			e.printStackTrace();
		}
		return list;
	}
