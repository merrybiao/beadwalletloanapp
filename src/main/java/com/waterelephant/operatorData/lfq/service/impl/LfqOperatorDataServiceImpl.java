package com.waterelephant.operatorData.lfq.service.impl;
...
import com.waterelephant.operatorData.lfq.mapper.LfqOperatorDataMapper;
import com.waterelephant.operatorData.service.impl.OperatorsDataAbstractService;
import com.waterelephant.utils.DateUtil;
@Service
public class LfqOperatorDataServiceImpl extends OperatorsDataAbstractService{

	private Logger logger = LoggerFactory.getLogger(LfqOperatorDataServiceImpl.class);
	
	@Autowired
	private LfqOperatorDataMapper lfqoperatordatamapper;

	@Override
	public OperatorUserDataDto getUserData(Long borrowerId, Long orderId) {
		logger.info("----【龙分期】依据:"+orderId+"查询基础数据执行开始----");
		return lfqoperatordatamapper.queryUserData(orderId);
	}
	...
	public List<OperatorTelDataDto> getTelDataList(Long borrowerId, Long orderId) {
		logger.info("----【龙分期】根据borrowerId:{},orderId:{}获取通话记录信息----",borrowerId,orderId);
		int months = 6;
		...
		try {
			while(months>0){
				...
				OperatorTelDataDto dto = lfqoperatordatamapper.getLfqCallCount(orderId, dateCount);
				if(null !=dto&&dto.getTotalSize()>0){
					...
					List<Map<String, String>> items = lfqoperatordatamapper.getLfqCallRecord(orderId, startTime, endTime);
					logger.info("----【龙分期】根据orderId:{},startTime:{},lastTime:{},查询bw_third_operate_voice表获取通话记录信息,结果为：{}",orderId,startTime,endTime,items == null ?"空":items.size());
			...
			logger.error("----【龙分期】根据borrowerId:{},orderId:{}获取通话记录信息异常：{}----",borrowerId,orderId,e.getMessage());
			e.printStackTrace();
		}
		return list;
	}
