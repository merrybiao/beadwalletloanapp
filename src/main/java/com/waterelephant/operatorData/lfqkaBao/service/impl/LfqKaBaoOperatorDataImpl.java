package com.waterelephant.operatorData.lfqkaBao.service.impl;
...
import com.waterelephant.operatorData.lfqkaBao.mapper.LfqKaBaoOperatorDataMapper;
...
@Service("lfqKaBaoOperatorDataImpl")
public class LfqKaBaoOperatorDataImpl extends OperatorsDataAbstractService{
	
	private Logger logger = LoggerFactory.getLogger(LfqKaBaoOperatorDataImpl.class);
	
	@Autowired
	private LfqKaBaoOperatorDataMapper lfqmapper;
		...
		try {
			while(months>0){
				...
				if(null !=dto&&dto.getTotalSize()>0){
					...
					logger.info("----【乐分期51卡宝】----根据orderId:{},startTime:{},endTime:{},查询bw_mf_call_info_record表获取通话记录信息,结果为：{}",orderId,startTime,endTime,items == null ?"空":items.size());
			...
			logger.error("----【乐分期51卡宝】----根据orderId:{}获取通话记录信息异常：{}----",orderId,e.getMessage());
			e.printStackTrace();
		}
		return list;
	}
	...
	public List<OperatorMsgDataDto> getMsgDataList(Long borrowerId, Long orderId) {
		logger.info("----【乐分期51卡宝】根据orderId:{}获取短信记录信息----",orderId);
		int months = 6;
		Calendar calendar = Calendar.getInstance(); 
		...
		try {
			while(months > 0){
				...
				OperatorMsgDataDto dto = lfqmapper.queryMsgCount(orderId, date);
				if(null != dto&&dto.getTotalSize() >0 ){
					calendar.set(Calendar.DATE, calendar.getActualMinimum(Calendar.DATE));
					//每月开始第一天时间
					String startTime = DateUtil.getDateString(calendar.getTime(), "yyyy-MM-dd") + " 00:00:00";
					...
					List<Map<String, Object>> msgrecord = lfqmapper.queryMsgData(orderId, startTime, endTime);
					logger.info("----【乐分期51卡宝】根据orderId:{},startTime:{},lastTime:{},查询bw_kabao_mx_sms表获取短信记录信息,结果为：{}",orderId,startTime,endTime,msgrecord == null ?"空":msgrecord.size());
			...
			logger.error("----【乐分期51卡宝】根据orderId:{}获取短信记录信息异常：{}----",orderId,e.getMessage());
			e.printStackTrace();
		}
		return list;
	}
	...
	public List<OperatorBillDataDto> getBillDataList(Long borrowerId,
			Long orderId) {
		logger.info("----【乐分期51卡宝】----根据orderId:{}获取账单记录信息----",orderId);
		Calendar calendar = Calendar.getInstance();
		...
		try {
			...
			List<OperatorBillDataDto>  list = lfqmapper.queryBillData(orderId, startTime, endTime);
			if(null == list || list.isEmpty()) list = lfqmapper.queryBillData(orderId, startTime, endTime);
			logger.info("----【乐分期51卡宝】----根据orderId:{},startTime:{},endTime:{},查询bw_kabao_mx_bill表获取账单记录信息,结果为：{}",orderId,startTime,endTime,list == null ?"空":list.size());
			...
			logger.error("----【乐分期51卡宝】----根据orderId:{}获取账单记录信息异常：{}----",orderId,e.getMessage());
			e.printStackTrace();
		}
		return null;
	}
	...
	public List<OperatorNetDataDto> getNetDataList(Long borrowerId, Long orderId) {
		logger.info("----【乐分期51卡宝】根据orderId:{}获取流量记录信息----",orderId);
		int months = 6;
		Calendar calendar = Calendar.getInstance();
		...
		try {
			while(months>0){
				...
				OperatorNetDataDto  dto = lfqmapper.queryNetCount(orderId, countTime);
				calendar.set(Calendar.DATE, calendar.getActualMinimum(Calendar.DATE));
				String startTime = DateUtil.getDateString(calendar.getTime(), "yyyy-MM-dd")+" 00:00:00";
				calendar.set(Calendar.DATE, calendar.getActualMaximum(Calendar.DATE));
				String endTime = DateUtil.getDateString(calendar.getTime(), "yyyy-MM-dd")+" 23:59:59";
				List<Map<String, Object>> items = lfqmapper.queryNetData(orderId, startTime, endTime);
				logger.info("----【乐分期51卡宝】根据orderId:{},startTime:{},lastTime:{},查询bw_kabao_mx_net表获取流量记录信息,结果为：{}",orderId,startTime,endTime,items == null ?"空":items.size());
				if(null != items&&items.size()>0){
					dto.setItems(items);
					list.add(dto);
				}
			...
			logger.error("----【乐分期51卡宝】根据orderId:{}获取流量记录信息异常：{}----",orderId,e.getMessage());
			e.printStackTrace();
		}
		return list;
	}
