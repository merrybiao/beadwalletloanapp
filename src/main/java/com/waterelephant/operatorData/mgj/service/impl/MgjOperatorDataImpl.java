package com.waterelephant.operatorData.mgj.service.impl;
...
import com.waterelephant.operatorData.mgj.mapper.MgjOperatorDataMapper;
import com.waterelephant.operatorData.service.impl.OperatorsDataAbstractService;
import com.waterelephant.utils.DateUtil;

@Service
public class MgjOperatorDataImpl extends OperatorsDataAbstractService{
	
	private Logger logger = LoggerFactory.getLogger(MgjOperatorDataImpl.class);
	
	@Autowired
	private MgjOperatorDataMapper mapper;
	...
	public List<OperatorTelDataDto> getTelDataList(Long borrowerId, Long orderId) {
		logger.info("----【优米管家】----根据orderId:{}获取通话记录信息----",orderId);
		int months = 6;
		...
		try {
			while(months>0){
				...
				if(null !=dto&&dto.getTotalSize()>0){
					...
					logger.info("----【优米管家】----根据orderId:{},startTime:{},endTime:{},查询bw_third_operate_voice表获取通话记录信息,结果为：{}",orderId,startTime,endTime,items == null ?"空":items.size());
			...
			logger.error("----【优米管家】----根据orderId:{}获取通话记录信息异常：{}----",orderId,e.getMessage());
			e.printStackTrace();
		}
		return list;
	}
	...
	public List<OperatorBillDataDto> getBillDataList(Long borrowerId,
			Long orderId) {
		logger.info("----【优米管家】----根据orderId:{}获取账单记录信息----",orderId);
		Calendar calendar = Calendar.getInstance();
		...
		try {
			...
			logger.info("----【优米管家】----根据orderId:{},startTime:{},endTime:{},查询bw_kabao_report_bill_info表获取账单记录信息,结果为：{}",orderId,startTime,endTime,list == null ?"空":list.size());
			...
			logger.error("----【优米管家】----根据orderId:{}获取账单记录信息异常：{}----",orderId,e.getMessage());
			e.printStackTrace();
		}
		return null;
	}
	...
	public List<OperatorMsgDataDto> getMsgDataList(Long borrowerId, Long orderId) {
		logger.info("----【优米管家】根据borrowerId:{},orderId:{}获取短信记录信息----",borrowerId,orderId);
		int months = 6;
		Calendar calendar = Calendar.getInstance(); 
		...
		try {
			while(months > 0){
				...
				if(null != dto&&dto.getTotalSize() >0 ){
					...
					logger.info("----【优米管家】根据orderId:{},startTime:{},lastTime:{},查询bw_jb_smses表获取短信记录信息,结果为：{}",orderId,startTime,endTime,jbmsgrecord == null ?"空":jbmsgrecord.size());
			...
			logger.error("----【优米管家】根据borrowerId:{},orderId:{}获取短信记录信息异常：{}----",borrowerId,orderId,e.getMessage());
			e.printStackTrace();
		}
		return list;
	}
	...
	public List<OperatorNetDataDto> getNetDataList(Long borrowerId, Long orderId) {
		logger.info("----【优米管家】根据borrowerId:{},orderId:{}获取流量记录信息----",borrowerId,orderId);
		int months = 6;
		Calendar calendar = Calendar.getInstance();
		...
		try {
			while(months>0){
				...
				logger.info("----【优米管家】根据orderId:{},startTime:{},lastTime:{},查询bw_jb_nets表获取流量记录信息,结果为：{}",orderId,startTime,endTime,items == null ?"空":items.size());
				if(null != items&&items.size()>0){
					dto.setItems(items);
					list.add(dto);
				}
			...
			logger.error("----【优米管家】根据borrowerId:{},orderId:{}获取流量记录信息异常：{}----",borrowerId,orderId,e.getMessage());
			e.printStackTrace();
		}
		return list;
	}
