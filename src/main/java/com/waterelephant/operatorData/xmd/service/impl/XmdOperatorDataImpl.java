package com.waterelephant.operatorData.xmd.service.impl;
...
import com.waterelephant.operatorData.xmd.mapper.XmdOperatorDataMapper;
import com.waterelephant.utils.DateUtil;
@Service(value = "xmdOperatorDataImpl")
public class XmdOperatorDataImpl extends OperatorsDataAbstractService{
	
	private Logger logger = LoggerFactory.getLogger(XmdOperatorDataImpl.class);
	
	@Autowired
	private XmdOperatorDataMapper mapper;

	@Override
	public OperatorUserDataDto getUserData(Long borrowerId, Long orderId) {
		logger.info("【星美贷】依据orderId:{}查询基础数据",orderId);
	...
	public List<OperatorMsgDataDto> getMsgDataList(Long borrowerId, Long orderId) {
		logger.info("----【星美贷】----根据orderId:{}获取短信记录信息<<<<<",orderId);
		int months = 6;
		...
		try {
			while(months > 0){
				...
				if(null != dto&&dto.getTotalSize() >0 ){
					...
					logger.info("----【星美贷】----根据orderId:{},startTime:{},endTime:{},查询bw_xmd_smses表获取短信记录信息,结果为：{}",orderId,startTime,endTime,msgrecord == null ?"空":msgrecord.size());
			...
			logger.error("----【星美贷】----根据orderId:{}获取短信记录信息异常：{}----",orderId,e.getMessage());
			e.printStackTrace();
		}
		return list;
	}
	...
	public List<OperatorTelDataDto> getTelDataList(Long borrowerId, Long orderId) {
		logger.info("----【星美贷】----根据orderId:{}获取通话记录信息----",orderId);
		int months = 6;
		Calendar calendar = Calendar.getInstance();
		...
		try {
			while(months>0){
				...
				if(null !=dto&&dto.getTotalSize()>0){
					...
					logger.info("----【星美贷】----根据orderId:{},startTime:{},endTime:{},查询bw_mf_call_info_record表获取通话记录信息,结果为：{}",orderId,startTime,endTime,items == null ?"空":items.size());
			...
			logger.error("----【星美贷】----根据orderId:{}获取通话记录信息异常：{}----",orderId,e.getMessage());
			e.printStackTrace();
		}
		return list;
	}
	...
	public List<OperatorBillDataDto> getBillDataList(Long borrowerId,Long orderId) {
		logger.info("----【星美贷】----根据orderId:{}获取账单记录信息----",orderId);
		Calendar calendar = Calendar.getInstance();
		...
		try {
			...
			logger.info("----【星美贷】----根据orderId:{},startTime:{},endTime:{},查询bw_xmd_bills表获取账单记录信息,结果为：{}",orderId,startTime,endTime,list == null ?"空":list.size());
			...
			logger.error("----【星美贷】----根据orderId:{}获取账单记录信息异常：{}----",orderId,e.getMessage());
			e.printStackTrace();
		}
		return null;
	}
	...
	public List<OperatorNetDataDto> getNetDataList(Long borrowerId, Long orderId) {
		logger.info("----【星美贷】----根据orderId:{}获取流量记录信息----",borrowerId,orderId);
		int months = 6;
		Calendar calendar = Calendar.getInstance();
		...
		try {
			while(months>0){
				...
				logger.info("----【星美贷】----根据orderId:{},startTime:{},endTime:{},查询bw_xmd_nets表获取流量记录信息,结果为：{}",orderId,startTime,endTime,items == null ?"空":items.size());
				if(null != items&&items.size()>0){
					dto.setItems(items);
					list.add(dto);
				}
			...
			logger.error("----【星美贷】----根据orderId:{}获取流量记录信息异常：{}----",orderId,e.getMessage());
			e.printStackTrace();
		}
		return list;
	}
