//
//package com.waterelephant.sxyDrainage.entity.kabao;
///**
// * ClassName:KaBaoIdentifyInfo <br/>
// * Function: 实名认证信息  <br/>
// * @author   liwanliang
// * @version   1.0  
// */
//public class KaBaoIdentifyInfo {
//	
//	private String frontFile; // 身份证正面照片url
//	private String backFile;  // 身份证反面照片url
//	private String natureFile;// 生活照url/手持照
//	private String nation;   // 民族
//	private String address;  // 身份证地址	
//	private String issuedBy; // 身份证发证处	
//	private String validDate;// 身份证有效期
//	public String getFrontFile() {
//		return frontFile;
//	}
//	public void setFrontFile(String frontFile) {
//		this.frontFile = frontFile;
//	}
//	public String getBackFile() {
//		return backFile;
//	}
//	public void setBackFile(String backFile) {
//		this.backFile = backFile;
//	}
//	public String getNatureFile() {
//		return natureFile;
//	}
//	public void setNatureFile(String natureFile) {
//		this.natureFile = natureFile;
//	}
//	public String getNation() {
//		return nation;
//	}
//	public void setNation(String nation) {
//		this.nation = nation;
//	}
//	public String getAddress() {
//		return address;
//	}
//	public void setAddress(String address) {
//		this.address = address;
//	}
//	public String getIssuedBy() {
//		return issuedBy;
//	}
//	public void setIssuedBy(String issuedBy) {
//		this.issuedBy = issuedBy;
//	}
//	public String getValidDate() {
//		return validDate;
//	}
//	public void setValidDate(String validDate) {
//		this.validDate = validDate;
//	}
//	@Override
//	public String toString() {
//		return "KaBaoIdentifyInfo [frontFile=" + frontFile + ", backFile=" + backFile + ", natureFile=" + natureFile
//				+ ", nation=" + nation + ", address=" + address + ", issuedBy=" + issuedBy + ", validDate=" + validDate
//				+ "]";
//	}
//}
//
