...
public class BankInfoUtils {
	...
	private final static String[] bankBin = { "621098", "622150", "622151", "622181", "622188", "955100", "621095",
			...
			"628267", "621392", "621481", "621310", "621396", "623251", "628351", "622586", "520152", "520382",
			"623668"};
	// "发卡行.卡种名称",
	private static final String[] bankName = { "邮政储蓄银行·绿卡通", "邮政储蓄银行·绿卡银联标准卡", "邮政储蓄银行·绿卡银联标准卡", "邮政储蓄银行·绿卡专用卡", "邮政储蓄银行·绿卡银联标准卡",
			...
			"东营莱商村镇银行·绿洲卡", "建设银行·单位结算卡", "玉溪市商业银行·红塔卡","广发银行·广发银行借记卡","广发银行·广发银行南航金卡","广发银行·广发银行南航普卡",
			"建设银行·借记卡"};

	public static Map<String, String> bankNameMap = new HashedMap();
	static {
		if (bankNameMap.size() == 0){
			for (int i = 0; i < bankBin.length; i++) {
				try {
					bankNameMap.put(bankBin[i], bankName[i]);
	...
	public static String getNameOfBank(String cardbin) {
		String name = bankNameMap.get(cardbin);
		if (name == null){
			name = RedisUtils.hget("bank_card_start_info", cardbin);
		}
		if (name == null){
			name = "";
		}
	...
	public static String getNameOfBankCard(String bankcard) {
		if (bankcard == null && bankcard.trim().length() < 6) {
			return "";
		}
		String cardbin = bankcard.substring(0, 6);
		return getNameOfBank(cardbin);
	}
}